...
class TestScript(unittest.TestCase):
    ...
    #     regStateList = data.registerStateLists[0]
    #     self.assertIsInstance(regStateList, RegisterStateList)
    #     self.assertEqual(len(regStateList.beforeStates), 1 * 16 + 1)
    #     self.assertEqual(len(regStateList.afterStates), 1 * 16 + 1)
    ...
    def testRunInstructionsX86(self):
        ...
        data: StateData = runInstruction.runInstructions(panda, instructions, n, verbose=True)
        self.assertEqual(len(data.registerStateLists), inst)
        for regStateList in data.registerStateLists:
            self.assertEqual(len(regStateList.bitmasks), n*16 + 1)
            self.assertEqual(len(regStateList.afterStates), n*16 + 1)
            self.assertEqual(len(regStateList.beforeStates), n*16 + 1)
