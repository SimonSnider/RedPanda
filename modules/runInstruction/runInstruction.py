...
def loadInstruction(panda: Panda, cpu, instruction, address=0):
    ...
    #load the instruction into the specified memory address and place a nop and jump instruction after it
...
def getNextValidReg(panda: Panda, startIndex):
    """
    Arguments:
    ...
    Output:
        ...
        valid registers
    """
    # Get the list of registers from panda
...
def runInstructions(panda: Panda, instructions, n, verbose=False):
    ...
    Outputs:
        returns a StateData object containing the instructions run and the register state lists for each instruction
    """
    ADDRESS = 0
    stateData = StateData()
