...
def on_insn1(cpu, pc):
    ...
    global count
    count += 1
    print("on_insn 1: " + str(count) + " / 4")
    if (count == 4):
        print("Disabling inst1 callback")
        panda.disable_callback("on_insn1")
        print("Enabling inst2 callback")
...
def on_insn2(cpu, pc):
    ...
    At each instruction, print capstone disassembly.
    '''
    global count, inst2count
    inst2count += 1
    print("on_insn 2: " + str(inst2count) + " / 4")
    if (inst2count > 3):
